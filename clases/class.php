...
class Trabajo extends Conexion{
    ...
    public function insertarUsuario(string $correo, string $contraseña, string $num_doc, string $tipo_doc, string $nombres, string $apellidos, string $telefono, string $direccion, string $tipo_usuario):int{
        $password=PASSWORD_HASH($contraseña,PASSWORD_DEFAULT,array("cost"=>14));
        $sql="INSERT INTO usuarios VALUES (:em, :pass, :img)";
    ...
    public function traerDatos($inicio,$resultados_por_pagina){
        //$pagina_actual
        $sql="SELECT * FROM persona ORDER BY nombres LIMIT :inicio,:resultados";
        $consult=$this->conexion->prepare($sql);
